...
class ConversationViewModel extends ChangeNotifier {
  ...
  // 缓存队列来存储加载期间的更新
  final List<NIMConversation> _pendingUpdates = [];

  // 缓存队列来存储加载期间的创建
  final List<NIMConversation> _pendingCreated = [];
  ...
  _init() {
    ...
    subscriptions
      ...
      // 如果正在加载，将更新缓存起来
      if (_isLoading) {
        _logI('Currently loading, caching updates');
        _pendingUpdates.addAll(event);
        return;
      }
    ...
    subscriptions
      ...
      if (event.type == NIMConversationType.team &&
        ...
        conversationInfo.haveBeenAit =
            AitServer.instance.isNewAitConversation(event);
      }
  ...
  _addItem(ConversationInfo conversationInfo, {bool needUpdate = true}) async {
  ...
  void handlePendingUpdate() {
    if (_pendingUpdates.isNotEmpty) {
      _logI('Processing ${_pendingUpdates.length} pending updates');
      List<ConversationInfo>? result = convertConversationInfo(_pendingUpdates);
      if (result != null) {
        for (var conversationInfo in result) {
          _updateItem(conversationInfo);
          _logI(
              'Processing pending update: ${conversationInfo.getConversationId()}');
        }
        doUnreadCallback();
      }
  ...
  // 处理加载期间缓存的创建
  void handlePendingCreated() async {
    if (_pendingCreated.isNotEmpty) {
      for (var event in _pendingCreated) {
        if (!(await _isValidConversation(event))) {
          deleteConversationById(event.conversationId);
          return;
        }
        ...
        _addItem(conversationInfo, needUpdate: false);
        if (event.type == NIMConversationType.p2p) {
          subscribeP2PUserStatus([conversationInfo]);
        }
        doUnreadCallback();
