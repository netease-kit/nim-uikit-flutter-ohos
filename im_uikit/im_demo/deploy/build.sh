...
if [ "$BACKUP_DIR" != "" ] && [ "$BACKUP_DIR" != "--" ]; then
    ...
    download_url=http://10.242.141.186/xkit/$BACKUP_DIR/im_demo/$VERSION_NAME/$platform_lowercase/$ARCHIVE_NAME
    qr_url="https://api.pwmqr.com/qrcode/create/?url=${download_url}"
    sh $PROJECT_PATH/build_tools/notification/notify_ex.sh --platform "$PLATFORM" --env "${ENV}" --version "${VERSION_NAME}" --mode team  --receiver "5058614" --downloadurl "${download_url}"  --installurl "${qr_url}"
    rm -rf build_tools
fi
## upload done
