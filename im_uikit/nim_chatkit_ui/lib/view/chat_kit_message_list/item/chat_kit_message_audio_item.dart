...
class ChatKitMessageAudioState extends State<ChatKitMessageAudioItem>
  ...
  void _startAudioPlay(NIMMessage message) async {
    isPlaying = true;
    _timer?.cancel();
    var attachment = message.attachment as NIMMessageAudioAttachment;
    if (attachment.url?.isNotEmpty == true && !needRenameAudioFile(message)) {
      _playAudio(UrlSource(attachment.url!), attachment.duration!);
    } else if (attachment.path != null && File(attachment.path!).existsSync()) {
      if (needRenameAudioFile(message)) {
        var newName = await renameFile(attachment.path!);
        attachment.path = newName;
      }
  ...
  //是否需要重命名音频文件
  //鸿蒙发来的音频文件，在iOS端需要重命名为.m4a格式
  bool needRenameAudioFile(NIMMessage message) {
    if (Platform.isIOS &&
        (message.attachment as NIMMessageAudioAttachment).path?.isNotEmpty ==
            true) {
      if (message.serverExtension?.isNotEmpty == true) {
        try {
          var serverExtension = message.serverExtension!;
          var extensionMap = jsonDecode(serverExtension);
          if (extensionMap[audioFromPlatform] == audioPlatformOhos) {
            return true;
          }
        } catch (e) {}
      }
  ...
  Future<String> renameFile(String oldName) async {
    try {
      // 新文件名修改后缀为.wav
      String newName = oldName.replaceAll(RegExp(r'\.mp4$'), '.m4a');
      // 创建File对象
