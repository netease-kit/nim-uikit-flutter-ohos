...
String _defaultPathNameBuilder(AssetPathEntity path) {
  if (path.isAll) {
    return S.of().imagePickerRecent;
  }

  // 使用 switch 语句提高可读性，并转换为小写以进行不区分大小写的比较
  switch (path.name.toLowerCase()) {
    case 'camera':
      return S.of().imagePickerCamera;
    case 'screenshots':
      return S.of().imagePickerScreenshots;
    default:
      return path.name;
  }
}
...
class AssetPickerConfig {
  const AssetPickerConfig({
    ...
    this.filterOptions,
    this.gridCount = 4,
    this.themeColor = const Color(0xFF337EFF),
    this.pickerTheme,
    this.textDelegate,
    ...
    this.pathNameBuilder = _defaultPathNameBuilder,
